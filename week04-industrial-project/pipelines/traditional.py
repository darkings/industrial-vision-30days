...
class TraditionalPipeline(BasePipeline):
    ...
    def run(self) -> PipelineOutput:
        ...
        threshold, opened_image = self._preprocess_image()
        t_preprocess_end = time.perf_counter()
        contours, detections = self._measure_contour_metrics(image=opened_image)
        execution_status, inspection_result, reason_codes = self._evaluate_ok_ng(
            detections=detections
        )
        t_inference_end = time.perf_counter()
        measurements = self._build_inspection_record(
            threshold=threshold, detections=detections, contours=contours
        )
        ...
        return PipelineOutput(record, annotated_image=None, debug_images={})
    ...
    def _preprocess_image(self) -> tuple:
        """灰度化、滤波、阈值分割、形态学处理图片"""
    ...
    def _measure_contour_metrics(
        ...
        for contour in contours:
            ...
            if area_ok and aspect_ratio_ok:
                ...
                center_offset_y = center_y - image_height / 2
                detections.append(
                    {
                        "bbox": [x, y, w, h],
                        "area": area,
