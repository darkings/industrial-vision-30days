...
def find_valid_contours(config, binary, image):
  ...
  min_area = config["contour_filter"]["min_area"]
  min_aspect_ratio = config["contour_filter"]["min_aspect_ratio"]
  max_aspect_ratio = config["contour_filter"]["max_aspect_ratio"]
...
def build_detection_result(
  config, tag, image, used_threshold, contours, valid_contours
):
  """构建检测结果
  ...
  if run_mode == "local":
    json_data = {"image_name": tag, **json_data}
...
def run_detection_pipeline(config, tag, image):
  """运行检测程序
  ...
  return gray, blur, binary, result, json_data, status
