...
## E022：把空字符 `\x00` 写成普通字符串 `x00`

日期与课程：2026-06-29，Day 8，海康 SDK 字符数组解码
...
这段代码语法正确，但不会按 C 字符串中的空字符截断。设备型号、序列号等字段后面可能残留大量 `\x00` 或无效字节，导致打印内容不干净，甚至影响后续字符串比较。

根本原因：

`b"x00"` 表示三个普通字节：字母 `x`、数字 `0`、数字 `0`。真正的空字符需要写成转义字节：
...
C 语言字符串常以 `\0` 作为结束符。海康 SDK 结构体里的型号、序列号字段通常是固定长度字符数组，需要在第一个空字符处截断，后面的填充内容不属于真实字符串。

以后如何避免：

- 写字节转义时确认是否带反斜杠。
- `b"x00"` 是普通文本；`b"\x00"` 才是十六进制空字节。
- 处理 SDK、串口、网络协议、二进制文件时，尤其要区分普通字符和转义字节。

## E023：创建/打开相机后没有更新资源状态变量

日期与课程：2026-06-29，Day 8，工业相机生命周期管理
...
相机句柄创建成功、设备打开成功后，`is_handle_created` 和 `is_device_opened` 仍然是 `False`。如果后续抓帧、转换或保存图片时发生异常，`finally` 中可能不会关闭设备、销毁句柄，导致相机被 Python 进程或 SDK 占用。

根本原因：

状态变量的作用是记录“资源是否已经成功申请”。申请成功后必须立即更新状态，否则释放逻辑不知道哪些资源需要释放。
...
工业相机 SDK 代码必须重视资源生命周期。创建句柄、打开设备、开始取流、获取 buffer 都属于资源申请；对应地要销毁句柄、关闭设备、停止取流、释放 buffer。

以后如何避免：

- 每次成功申请资源后，立刻更新对应状态变量。
- `finally` 中只根据状态变量释放已经申请成功的资源。
- 相机、文件、网络连接、线程、数据库连接都适用同样原则。

## E024：实验结果来源没有标清

日期与课程：2026-06-29，Day 9，真实相机图像接入 OpenCV

错误类型：实验记录与结果追溯错误

错误现象：

把示范脚本运行结果直接写成“本节实验结果”，但学员自己的独立练习代码还没有完成。
...
工业视觉项目中，实验结果必须能追溯来源。如果不标明结果来自示范脚本、独立练习脚本、某次参数实验或某次现场采集，后续复盘时会分不清数据依据。
...
1. 把示范结果误认为独立练习结果。
2. 把旧参数结果误认为新参数结果。
3. 后续调试阈值、曝光、轮廓数量时依据混乱。
4. 现场报告无法追溯图片、参数和代码版本。
```
...
以下结果来自 `test01_camera_image_analysis.py`。

## 独立练习结果

以下结果来自 `test02_camera_image_analysis_practice.py`。
```

核心记忆：

```text
实验结果必须写清楚来源：脚本名、输入图片、关键参数和输出时间。
```
