...
OUT_DIR = BASE_DIR / "outputs" / "prepare_ocr_roi"
...
def print_roi_shape_and_otsu_threshold(roi_image, inverse, threshold):
  """
  打印roi.shape 和 otsu阈值
  """
  print(f"roi_image的shape为{roi_image.shape}")
  print(f"当前roi图像二值化为：{'反二值化' if inverse else '正二值化'}")
...
def main():
  ...
  image_paths = save_outputs(OUT_DIR, roi_image, gray_image, binary_image)
  print_roi_shape_and_otsu_threshold(roi_image, inverse, threshold)
  ...
  save_json(OUT_DIR, "summary", summary_json_data)
