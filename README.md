...
| Day 10 | 稳定成像系统搭建           | 已完成 | 支架、环形光源、光源控制器到位后，完成曝光、丢帧和成像基线 |
| Day 11 | 真实相机检测小流水线       | 已完成 | 轮廓检测、OK/NG、JSON 输出、函数化整理和多帧稳定性验证     |
| Day 12 | 检测流程封装与配置化       | 下一课 | 封装 pipeline、配置化参数、批量处理真实样本                 |
...
1. `AGENTS.md`：当前协作规则、文档更新规则和下一课入口。
2. `30_DAY_LEARNING_PLAN.md`：当前 30 天路线和 Day12-Day16 节奏。
3. Obsidian 中当天的理论笔记。
4. 对应日期目录中的练习代码和 outputs。

更新说明：从 2026-07-04 起，日常学习入口和文档更新规则以根目录 `AGENTS.md` 为准。`LEARNING_PROGRESS.md` 已删除；`LEARNING_ERRORS.md` 继续作为个人错误知识库维护。
...
Day 12
检测流程封装、配置化与批量验证
下一步：先做 Day12 第 1 节，基于 Day11 检测流程整理可复用 pipeline。
```
...
Day01-Day11 已完成。
Day10 已完成稳定成像系统搭建。
Day11 已完成真实相机检测小流水线和多帧稳定性验证。
下一步从 Day12 开始整理 pipeline、配置化参数和批量验证。
```
...
6. 再按 AGENTS.md 和 30_DAY_LEARNING_PLAN.md 中的当前入口继续。
```

当前已完成的 Day10/Day11 边界：

```text
Day10 已完成固定支架、相机、镜头、环形光源和光源控制器。
Day11 已完成基础轮廓检测、OK/NG、JSON 输出和多帧稳定性验证。
Day12 开始不再继续堆临时脚本，而是把检测流程整理成可复用 pipeline。
```
...
Day09 已完成固定图片下的阈值理解，Day10 已完成稳定成像系统搭建，Day11 已完成基础检测流水线和多帧稳定性验证。后续从 Day12 开始，将检测流程封装为可复用 pipeline，并逐步补充 ROI、模板匹配和批量验证。
...
环境或路径规则变化
项目级协作规则变化
用户明确要求更新 README / 30_DAY_LEARNING_PLAN / LEARNING_ERRORS
```
