...
```text
Day16：YOLO 标注规范与小数据集准备
当前路线：直接采集和标注 dpi_button，不再先做 mouse 数据集。
当前标注：CVAT 标注已完成，YOLO/Ultralytics 标签已导出并通过格式检查。
下一步：做 YOLO 数据集加载验证和小规模训练验证。
```

当前 YOLO 数据集位置：

```text
D:\Projects\industrial-vision-30days\week03-yolo\day16\dataset
```
...
dataset/
  images/
    train/   23 张 dpi_button 图片
    val/      5 张 dpi_button 图片
  labels/
    train/   23 个同名 .txt
    val/      5 个同名 .txt
  dataset.yaml
...
```text
D:\Projects\industrial-vision-30days\week03-yolo\day16\raw_samples\dpi_button
```

样本拆分记录和缩略图总览：

```text
D:\Projects\industrial-vision-30days\week03-yolo\day16\outputs\dpi_button_split_manifest.json
D:\Projects\industrial-vision-30days\week03-yolo\day16\outputs\dpi_button_contact_sheet.png
D:\Projects\industrial-vision-30days\week03-yolo\day16\outputs\dpi_button_annotation_check_summary.json
D:\Projects\industrial-vision-30days\week03-yolo\day16\outputs\dpi_button_all_label_preview.png
```
...
换电脑后如果 `ultralytics` 不可用，先在项目 `.venv` 中执行：
