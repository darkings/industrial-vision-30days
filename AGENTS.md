...
6. 助手准备讲解的新内容已经偏离 LEARNING_PLAN.md 的天级安排，或把后续天数的内容提前/延后讲解。
```
...
```text
当前状态：已删除。
原因：进度信息改由 AGENTS.md、LEARNING_PLAN.md 和 Obsidian 当天笔记维护。
```
...
文章头部 H1 上方不要有空行。
YAML 后直接接文章 # H1 标题，不留空行。
正文 H2-H6 标题上方必须有一个空行。
所有 H1-H6 标题下方如果紧跟正文、列表、代码块，不要有空行。
```
